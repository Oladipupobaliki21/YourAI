...
function initFirebase() {
  if (typeof window.FIREBASE_CONFIG !== 'undefined' && window.FIREBASE_CONFIG && window.FIREBASE_CONFIG.apiKey && window.FIREBASE_CONFIG.apiKey !== 'YOUR_FIREBASE_API_KEY') {
...
async function loadState() {
  ...
  try {
    ...
    if (userStr) {
      ...
      if (currentUserEmail) {
        APP_KEY = 'yourAI_app_' + currentUserEmail;
      }
    }
  } catch (e) { }
  ...
  if (db && currentUserEmail) {
    try {
      const doc = await db.collection('user_states').doc(currentUserEmail).get();
      if (doc.exists) {
        const data = doc.data();
        if (data.conversations) state.conversations = data.conversations;
        if (data.settings) state.settings = { ...state.settings, ...(data.settings || {}) };
        if (data.userProfile) state.userProfile = { ...state.userProfile, ...(data.userProfile || {}) };
      }
    } catch (e) {
  ...
  if (!['gemini-2.0-flash', 'gemini-2.5-flash'].includes(state.settings.model)) {
...
function saveState() {
  try {
    ...
    localStorage.setItem(APP_KEY, JSON.stringify(dataToSave));
    ...
    if (db && currentUserEmail) {
      db.collection('user_states').doc(currentUserEmail).set(dataToSave, { merge: true })
        .catch(e => console.warn('Error saving to Firebase:', e));
    }
  } catch (e) {
  ...
  wrapper.innerHTML = `
    ...
    <div class="msg-content">
      ...
      <div class="msg-actions">
        ...
        ${!isUser ? `<button class="msg-action-btn speak-btn" onclick="speakMessage('${msg.id}', this)">
...
let currentSpokenMsgId = null;

function speakMessage(msgId, btnEl) {
  const conv = getCurrentConversation();
  if (!conv) return;
  const msg = conv.messages.find(m => m.id === msgId);
  if (!msg) return;

  if (currentSpokenMsgId === msgId && speechSynth && speechSynth.speaking) {
    stopSpeaking();
    return;
  }

  stopSpeaking();
  currentSpokenMsgId = msgId;
  const textToSpeak = msg.content.replace(/[#*`]/g, '');
  ...
  const theBtn = btnEl || document.querySelector(`.msg[data-id="${msgId}"] .speak-btn`);

  if (theBtn) {
    theBtn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg> Stop`;
  }

  utter.onend = () => {
    if (currentSpokenMsgId === msgId) {
      currentSpokenMsgId = null;
      if (theBtn) {
        theBtn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>\n          Speak`;
      }
    }
  };
...
function stopSpeaking() {
  ...
  if (currentSpokenMsgId) {
    const theBtn = document.querySelector(`.msg[data-id="${currentSpokenMsgId}"] .speak-btn`);
    if (theBtn) {
      theBtn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>\n          Speak`;
    }
    currentSpokenMsgId = null;
...
function startCallMic() {
  ...
  callRecognition.onresult = async (event) => {
    ...
    try {
      ...
      if (state.callSpeakerActive) {
        const utter = speakText(shortResponse);
        if (utter) {
          utter.onend = () => { document.getElementById('callSubStatus').textContent = 'Tap mic to speak again'; };
        } else {
      ...
      if (state.callSpeakerActive) speakText('Sorry, there was an error processing your request.');
  ...
  callRecognition.onend = () => {
    ...
    if (document.getElementById('callSubStatus').textContent === 'Listening...') {
      document.getElementById('callSubStatus').textContent = 'Tap mic to speak again';
    }
  };
...
async function generateImage() {
  ...
  try {
    ...
    if (!state.currentConversationId) createConversation(`[Image] ${prompt}`);
    addMessage('user', `Generate image: ${prompt}`);
    addMessage('assistant', `![Generated Image](${imageUrl})`);
    filterRecentChats();
...
Format the email properly with Subject line, greeting, body, and sign-off.`;
  ...
  try {
    ...
    if (!state.currentConversationId) createConversation(`[Email] ${type}`);
    addMessage('user', `Draft email: ${context}`);
    addMessage('assistant', result);
    filterRecentChats();
...
Include: Hook, intro, main sections with timestamps, transitions, B-roll suggestions, and outro.`;
  ...
  try {
    ...
    if (!state.currentConversationId) createConversation(`[Video] ${topic}`);
    addMessage('user', `Write video script: ${topic}`);
    addMessage('assistant', result);
    filterRecentChats();
...
function switchMode(mode) {
  ...
  inputArea.style.display = (mode === 'chat') ? 'block' : 'none';
...
function saveSettings() {
  const apiKeyInput = document.getElementById('apiKeyInput');
  if (apiKeyInput) state.settings.apiKey = apiKeyInput.value.trim();
...
function renameChat(id, e) {
  ...
  if (!conv) return;
  const newName = prompt('Rename chat:', conv.title);
  if (newName && newName.trim()) {
    conv.title = newName.trim();
    saveState();
    filterRecentChats();
    renderHistoryPanel();
    if (state.currentConversationId === id) updatePageTitle(conv.title);
  }
}
...
function shareChat(id, e) {
  ...
  if (!conv) return;
  if (navigator.share) {
    navigator.share({ title: conv.title, text: `Check out this AI conversation: ${conv.title}` }).catch(console.error);
  } else {
    navigator.clipboard.writeText(`Chat: ${conv.title}`);
    showToast('Chat info copied to clipboard!', 'success');
  }
}
...
function renderRecentChats(searchQuery = '') {
  const el = document.getElementById('recentChats');
  if (!el) return;

  const currentSearch = document.getElementById('sidebarSearch')?.value.toLowerCase() || '';
  const queryToUse = searchQuery || currentSearch;

  let recent = state.conversations;
  if (queryToUse) {
    recent = recent.filter(c => c.title.toLowerCase().includes(queryToUse));
  } else {
    recent = recent.slice(0, 12);
  }

  if (recent.length === 0) {
    el.innerHTML = `<p style="font-size:0.75rem;color:var(--text-dim);padding:8px 10px;">No chats found</p>`;
    return;
  }
  ...
  el.innerHTML = recent.map(conv => `
    <div class="history-item ${conv.id === state.currentConversationId ? 'active' : ''}" onclick="loadConversation('${conv.id}')">
      ...
      <div class="history-actions-wrap" onclick="event.stopPropagation()">
        <button class="history-dots" onclick="toggleHistoryMenu('${conv.id}', event)" title="Options">•••</button>
        <div class="history-menu" id="menu-${conv.id}">
          <button onclick="renameChat('${conv.id}', event)">
          ...
          <button onclick="shareChat('${conv.id}', event)">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
            Share
          </button>
          <button class="danger" onclick="deleteConversation('${conv.id}', event)">
  ...
  el.innerHTML = state.conversations.map(conv => `
    <div class="history-card" onclick="loadConversation('${conv.id}')">
      ...
      <div class="history-card-actions">
        <button class="history-card-del" onclick="renameChat('${conv.id}', event)">Rename</button>
        <button class="history-card-del" onclick="shareChat('${conv.id}', event)">Share</button>
        <button class="history-card-del" style="color:var(--red);border-color:var(--border);" onclick="deleteConversation('${conv.id}', event)">Delete</button>
      </div>
    </div>
  `).join('');
}
